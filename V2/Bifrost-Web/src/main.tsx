    ...
    <main className="min-h-screen bg-[#07111d] text-slate-100">
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-8 md:px-10">
        ...
        {session.status === "authenticated" ? (
          workspace === "profile" ? <ProfileWorkspace accessToken={session.accessToken} currentUser={session.user} /> : workspace === "requests" ? <RequestWorkspace accessToken={session.accessToken} /> : workspace === "shop" ? (hasShopAccess ? <ShopWorkspace accessToken={session.accessToken} /> : <NoAccessWorkspace user={session.user} />) : workspace === "comms" ? (hasCommsAccess ? <CommsWorkspace accessToken={session.accessToken} /> : <NoAccessWorkspace user={session.user} />) : workspace === "transport" ? (hasTransportAccess ? <TransportWorkspace accessToken={session.accessToken} /> : <NoAccessWorkspace user={session.user} />) : workspace === "vehicles" ? (hasVehicleAccess ? <VehicleWorkspace accessToken={session.accessToken} /> : <NoAccessWorkspace user={session.user} />) : !hasLogisticsAccess ? <NoAccessWorkspace user={session.user} /> : workspace === "locations" ? <LocationWorkspace accessToken={session.accessToken} /> : workspace === "warehouse" ? <WarehouseWorkspace accessToken={session.accessToken} /> : workspace === "loans" ? <LoanWorkspace accessToken={session.accessToken} /> : workspace === "private-equipment" ? <PrivateEquipmentWorkspace accessToken={session.accessToken} /> : <EquipmentWorkspace user={session.user} accessToken={session.accessToken} />
...
type Workspace = "equipment" | "warehouse" | "locations" | "loans" | "private-equipment" | "requests" | "vehicles" | "transport" | "comms" | "shop" | "profile";
