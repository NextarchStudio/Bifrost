...
assert(webClient?.publicClient === false, "bifrost-web må være en confidential klient.");
assert(webClient?.clientAuthenticatorType === "client-secret", "bifrost-web må bruke client secret server-side.");
assert(webClient?.secret === "bifrost-local-development-only", "Forventet dokumentert lokal utviklingssecret.");
...
assert(requiredWebOrigins.every((origin) => webClient?.redirectUris?.includes(`${origin}/auth/callback`)), "Alle tre eksakte Bifrost redirect-URI-er må være registrert.");

console.info("Local infrastructure config valid: bifrost-local, 11 roles, confidential PKCE client, no users.");
