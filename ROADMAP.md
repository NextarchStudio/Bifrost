...
| Fase 2 – identitet | Implementert | Obligatorisk Keycloak/OIDC støtter confidential klient med server-side kodeutveksling, PKCE S256, state, nonce, JWT/JWKS-validering, `HttpOnly` Bifrost-sesjon og automatisk V1-brukerprovisjonering. Client secret og Keycloak-token eksponeres ikke til Web. Lokal V1-innlogging er bevart som databasekontrollert reserve med Argon2id-verifisering, hash-lagrede V2-sesjoner, ratebegrensning og tokenfri audit. Tilgangsmatrisen dekker alle 11 V1-roller; representative flerrollekombinasjoner skal fortsatt verifiseres i staging. |
...
| Fase 6 – oppgaver, feedback og admin | Implementert, ikke staging-verifisert | Oppgaver, feedback/varsler, admin, regelstyrt brukerprovisjonering fra Crew API, valgfri Worker-basert velkomst-e-post, statistikk, kontrollert crew-/brukerreset, dashboard og globalt søk er implementert i API og Web. Paritetstest mot anonymiserte stagingdata gjenstår. |
...
**Status:** Implementert, ikke staging-verifisert. Oppgaver er implementert i API og Web med egen-/lederinnsyn, tildeling, prioritet, frist, status og valgfri transportkobling. Feedback, lokalt lagrede bildevedlegg og globale statusvarsler er implementert med V1-rollene og eksisterende tabeller. Kjerneadmin for brukere, roller, aktiv-status, kompetanser og krypterte innstillinger er levert. Badge-scan kan opprette eller synkronisere brukere fra Crew API når en aktiv crew-/crewrolle-regel matcher; flere regler kan tildele flere eksisterende V1-roller uten å fjerne manuelle roller. Velkomst-e-post for helt nye brukere kan styres av en developer-bryter og sendes asynkront via Bifrost-Worker. Alle V1-statistikkgrupper leses direkte fra eksisterende tabeller og vises i adminflaten. V1s destruktive crew-/brukerreset er bevart med developer-only forhåndsvisning, eksakt bekreftelsesfrase, kontroll av bruker-ID 2 og transaksjonelle slettinger. Dashboardet er startsiden for alle innloggede, mens globalt søk beholder V1s fire roller og søkefelt. Paritetstest mot anonymiserte stagingdata gjenstår.

**Leveranser**

- Oppgaver og status
- Feedback, vedlegg og notifications
- Admin for brukere, roller, Crew-regler, e-postbryter, innstillinger og crew-cache
...
Web bruker kun API-lenke og offentlig klientkonfigurasjon. Hemmelige API-nøkler, Keycloak client secret og Keycloak-token skal ikke bygges inn i eller returneres til browser-bundlen. API-et utfører confidential OIDC-kodeutveksling server-side og utsteder en hash-lagret, `HttpOnly` Bifrost-sesjon. API får database- og tekniske runtime-hemmeligheter fra env/secret store. OIDC-endepunkter, client-id, systeminnstillinger, integrasjonsnøkler, feature flags og rollemetadata leses fra DB.
